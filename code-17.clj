...
(defn shoot-y [y [adjx adjy] xrange yrange]
  (let [ra (iterate-shoot [0 0] [adjx y] xrange yrange 0)
    ...
    (cond 
      ...
      :else
      (count-best (for [x (range adjx adjy)]
        (iterate-shoot [0 0] [x y] xrange yrange 0))))))
...
(defn solve [xrange yrange y current-best]
  (let [res (shoot-y y [-400 400] xrange yrange)]
    (do
      ...
      (condp = res
        ...
        (recur xrange yrange (inc y) (+ current-best res))))))

;(prn (solve [20 30] [-10 -5] -20 0))
(prn (solve [70 125] [-159 -121] -1500 0))
